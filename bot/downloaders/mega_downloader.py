...
Ported from NEO-WZML with NXTL adapter wrapper.
Supports file and folder downloads, file selection, queue management.

Original: github.com/irisXDR/NEO-WZML
Adapted for: NXTL
"""
# ── NXTL Adapter ──────────────────────────────────────────────
async def mega_download(link: str, dest_dir: str, listener) -> None:
    """
    NXTL-compatible wrapper for NEO-WZML's add_mega_download.
    Listener must implement on_download_error, on_download_start,
    on_download_complete, and expose .link, .name, .size, .is_cancelled.
    """
    listener.link = link
    await add_mega_download(listener, dest_dir)
